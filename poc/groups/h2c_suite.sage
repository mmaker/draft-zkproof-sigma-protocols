...
BasicH2CSuiteDef = namedtuple(
    "BasicH2CSuiteDef", "E F Aa Bd expand H L MapT h_eff k is_ro dst")
...
class BasicH2CSuite(object):
    ...
    def hash_to_field(self, msg, count):
        xi_vals = hash_to_field(
            msg, count, self.p, self.m, self.L, self.expand)
...
class IsoH2CSuite(BasicH2CSuite):
    def __init__(self, name, sdef):
        ...
        super(IsoH2CSuite, self).__init__(
            name, sdef.base._replace(Aa=sdef.Ap, Bd=sdef.Bp))

        # check that we got a reasonable isogeny map
        self.iso_map = sdef.iso_map
        assert self.iso_map.domain() == EllipticCurve(
            self.F, [sdef.Ap, sdef.Bp]), "isogeny map domain mismatch"
        assert self.iso_map.codomain() == EllipticCurve(
            self.F, [sdef.base.Aa, sdef.base.Bd]), "isogeny map codomain mismatch"
...
class MontyH2CSuite(BasicH2CSuite):
    def __init__(self, name, sdef):
        ...
        super(MontyH2CSuite, self).__init__(
            name, sdef._replace(MapT=GenericEll2))
...
class EdwH2CSuite(MontyH2CSuite):
    def __init__(self, name, sdef):
        ...
        super(EdwH2CSuite, self).__init__(
            name, sdef.base._replace(Aa=sdef.Ap, Bd=sdef.Bp))
        ...
        self.to_self = lambda P: self.edwards(
            *sdef.rational_map(self.monty.to_self(P)))
